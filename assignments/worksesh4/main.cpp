...
void render(ew::Shader defShader, ew::Model model, GLuint texture)
{
	glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer.fbo);
	{
		...
		for (auto i = 0; i <= 10; i++)
		{
			for (auto j = 0; j <= 10; j++)
			{
				defShader.setMat4("_Model", glm::translate(glm::vec3(i * 4.0f, 0.0f, j * 4.0f)));
				model.draw();
			}
		}
	}
	...
	//		for (int j = 0; j <= 10; j++)
	//		{
	//			defShader.setMat4("_CameraViewProj", camera.projectionMatrix() * camera.viewMatrix());
	//			defShader.setVec3("_Color", glm::vec3(1.0, 0.0, 0.0));
	//			defShader.setMat4("_Model", glm::translate(glm::vec3(i * 2.0f, 5, j * 2.0f)));
	//			sphere.draw();
